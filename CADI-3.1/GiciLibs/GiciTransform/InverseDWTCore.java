...
public class InverseDWTCore{
	...
	public void setParameters(int WTTypes, int WTAxis){
		...
		/* Setting the values of the region to compute the whole image */
	...
	public void run() throws ErrorException{
		...
		if( WTTypes != 0 ){
			//Size to detransform for each axis.
	...
	private float[] oddEvenFiltering(float[] src, int WTTypes) throws ErrorException{
		...
		case 7:
			...
			dst[subbandSize-1] = dst[subbandSize-1]; // last sample is only copied
			break;
		case 8:
			// Reversible Haar (S-transform)
			float s = 0;
