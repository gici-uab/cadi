...
public class ViewWindowField {
  ...
  public int roundDirection = ROUND_DOWN;
  ...
  /**
	 * Constructor (copy).
	 * 
	 * @param viewWindow object to be copied.
	 */
  public ViewWindowField(ViewWindowField viewWindow) {
    ...
    this.fsiz[0] = viewWindow.fsiz[0];
    this.fsiz[1] = viewWindow.fsiz[1];
    this.roundDirection = viewWindow.roundDirection;
    this.roff[0] = viewWindow.roff[0];
    this.roff[1] = viewWindow.roff[1];
    this.rsiz[0] = viewWindow.rsiz[0];
    this.rsiz[1] = viewWindow.rsiz[1];
    if (viewWindow.comps != null) {
      comps = new int[viewWindow.comps.length][2];
      for (int i = 0; i < viewWindow.comps.length; i++) {
        comps[i][0] = viewWindow.comps[i][0];
        comps[i][1] = viewWindow.comps[i][1];
      }
    }
    this.layers = viewWindow.layers;
  }
  ...
  public ViewWindowField(int[] fsiz, int[] roff, int[] rsiz, int[][] comps, int[][] stream, String context, float srate, String roi, int layers) {
    this(fsiz, ROUND_DOWN, roff, rsiz, comps, stream, context, srate, roi, layers);
  }
  ...
  /**
	 ...
	 * 
	 * @param fsiz definition in {@link #fsiz}
	 * @param roundDirection definition in {@link #roundDirection}
	 * @param roff definition in {@link #roff}
	 * @param rsiz definition in {@link #rsiz}
	 * @param comps definition in {@link #comps}
	 * @param stream definition in {@link #stream}
	 ...
	 * @param layers definition in {@link #layers} 
	 */
  public ViewWindowField(int[] fsiz, int roundDirection, int[] roff, int[] rsiz, int[][] comps, int[][] stream, String context, float srate, String roi, int layers) {
    this.fsiz[0] = fsiz[0];
    this.fsiz[1] = fsiz[1];
  ...
  public static void deepCopy(ViewWindowField viewWindow1, ViewWindowField viewWindow2) {
    ...
    viewWindow2.roundDirection = viewWindow1.roundDirection;
  ...
  /**
	 ...
	 * 
	 * @param viewWindow data to be copied.
	 */
  public static ViewWindowField copyOf(ViewWindowField viewWindow) {
    return new ViewWindowField(viewWindow);
  }
   ...
   * @return 
   */
  public boolean equals(ViewWindowField woi) {
    if ((fsiz[0] != woi.fsiz[0]) || (fsiz[1] != woi.fsiz[1])) {
      return false;
    }
    if ((roff[0] != woi.roff[0]) || (roff[1] != woi.roff[1])) {
      return false;
    }
    if ((rsiz[0] != woi.rsiz[0]) || (rsiz[1] != woi.rsiz[1])) {
      return false;
    }
    if ((comps == null) && (woi.comps != null)) {
      return false;
    } else if ((comps != null) && (woi.comps == null)) {
      return false;
    } else if ((comps == null) && (woi.comps == null)) {
      // do nothing
    } else {
      if (comps.length != woi.comps.length) {
        return false;
      }
      for (int i = 0; i < comps.length; i++) {
        if ((comps[i][0] != woi.comps[i][0])
                || (comps[i][1] != woi.comps[i][1])) {
          return false;
        }         
      }
    }
    if (layers != woi.layers) {
      return false;
    }
  ...
  public void list(PrintStream out) {
    ...
    // VIEW WINDOW FIELD
    if (fsiz[0] != -1) {
      out.print("fsiz: [" + fsiz[0] + "," + fsiz[1]);
      switch (roundDirection) {
        case ROUND_DOWN:
          out.println(",round-down]");
          break;
        case ROUND_UP:
          out.println(",round-up]");
          break;
        case CLOSEST:
          out.println(",closest]");
          break;
