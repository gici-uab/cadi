...
public class JPIPResponseFields {
	 ...
	 * conformant manner. Legal capabilities are the same as those defined for the Capability request field in {@link ClientCapPrefField}
	 */
  ...
  // ============================= public methods ==============================
	 ...
	 * 
	 * @return the response view window.
	 */
	...
	public void setViewWindow(ViewWindowField viewWindow) {
		...
		// stream
		if (viewWindow.stream != null) stream = viewWindow.stream;
		
		// context
		if (viewWindow.context != null) context = viewWindow.context;
		
		// srate
		srate = viewWindow.srate;
		
		// roi
		if (viewWindow.roi != null) roi = viewWindow.roi;

		// layers
		if (viewWindow.layers != -1) layers = viewWindow.layers;
	}
  ...
  @Override
	...
	public void list(PrintStream out) {
		...
		if (port != -1) out.println("   port: "+ port);
		if (auxport != -1) out.println("   auxport: " + auxport);

		if (qid != -1) out.println("quid: " + qid);
		...
		// METADATA FIELD
		if (metareq != null) out.println("metare:= " + metareq);

		// DATA LIMIT FIELD

		if (len != -1 ) out.println("len: " + len);

		if (quality != -1) out.println("quality: " + quality);
		...
		// CACHE MANAGEMENT FIELD
		if ( mset != null ) out.println("mset: " + mset);

		// UPLOAD FIELD

		//CLIENT CAPABILITIES PREFERENCES FIELD
		if ( cap != null ) out.println("cap: " + cap);

		if (pref != null) out.println("pref: " + pref);
	}	
