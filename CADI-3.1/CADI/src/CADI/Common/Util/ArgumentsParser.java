...
public class ArgumentsParser {
      ...
      if (argFound != -1) {
        ...
        while (argOptions < args.length) {
          if (argFind(args[argOptions]) != -1) {
            break;
          } else if (args[argOptions].startsWith("-")) {
            break;
          } else {
            argOptions++;
          }
  ...
  public void showArgsInfo() {
    ...
    for (int numArg = 0; numArg < argumentsSpecification.length; numArg++) {
      ...
      if (argumentsSpecification[numArg][6] != "") {
        System.out.println("  Explanation:\n    " + argumentsSpecification[numArg][6]);
      }
      if (argumentsSpecification[numArg][3] != "") {
  ...
  public long parseRate(String option) throws ParameterException {
    long maxBandwidth = -1;
    ...
    if (option.charAt(index) == 'K') {
      multiplier *= 1L << 10;
    } else if (option.charAt(index) == 'M') {
      multiplier *= 1L << 20;
    } else if (option.charAt(index) == 'G') {
      multiplier *= 1L << 30;
    } else if (option.charAt(index) == 'T') {
      multiplier *= 1L << 40;
    }
    ...
    maxBandwidth = Integer.parseInt(option.substring(0, index)) * multiplier;
